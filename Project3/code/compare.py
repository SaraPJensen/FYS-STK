...
def main():
    ...
    for time1, time2 in zip([ta, tb], [tc, td]):
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=x1[time1], y=expl_sol1[time1], mode="lines", name="Explicit solver", line=dict(width=5)))
        fig.add_trace(go.Scatter(x=x2[time2], y=expl_sol2[time2], mode="lines", name="Explicit solver with smaller step size", line=dict(width=5)))
        fig.add_trace(go.Scatter(x=x2[time2], y=ana_sol[time2], mode="lines", name="Analytic solution", line=dict(width=5)))
        fig.add_trace(go.Scatter(x=x1[time1], y=NN_sol[time1], mode="lines", name="Neural solver", line=dict(width=5)))
        fig.show()
