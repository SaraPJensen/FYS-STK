...
def const(t): return 1
def lin_dec(t): return 1 - t
def parabola(t): return 0.3 * t**4 - t**3 + 0.6 * t + 0.9
def cos_up(t): return 2 - np.cos(t)
def cos_down(t): return np.cos(t)


funcs = [const, lin_dec, parabola, cos_up, cos_down]
names = ["Constant", "Linear decrease",
         "Parabola", "Increasing cos", "Decreasing cos"]
...
def main():
    ...
    for f, name in zip(funcs, names):
        ...
        cost.add_trace(go.Scatter(x=Solver.t[int(epochs/5):], y=Solver.history["cost"][int(
            epochs/5):], mode="lines", name=name, line=dict(width=5)))
        ls.add_trace(go.Scatter(
            x=Solver.t, y=Solver.history["eta"], mode="lines", name=name, line=dict(width=5)))
