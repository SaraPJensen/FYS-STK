...
None of these leraning schedules takes into account the history of learning. An adaptive learning rate could change according to what would give faster convergence in the moment. This is left for future work on the matter.

\begin{figure}[tb]
	\minipage{0.49\textwidth}
	\includegraphicsmaybe[width=\linewidth]{NN_ls_shapes.png}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	\includegraphicsmaybe[width=\linewidth]{NN_cost_ls.png}
	\endminipage\hfill
...
Now, the solution from the neural network trained above was compared to the explicit solver with both $\Delta x=0.1$ and $\Delta x=0.01$, and the analytical solution. 3 points in time was chosen, and the 4 solutions was plotted as function of $x$. These are shown in Figure \ref{fig:compare}. At all times the solution from the explicit solver with $\Delta x=0.01$ was indistinguashible from the analytic solution. The other explicit solver have too low values, with the relative error increasing with time, which is as expected. At the time, the neural solver was more accurate than one of the explicit solvers, with a low relative error. However, at the next time step, it was worse, giving too high solution. At $T=1.5$, the analytical solution was nearly flat, as seen at the magnitude of the curve. Both the explicit solvers gave results as expected. The neural network however did not, being too high, and not symmetic. While the absolute error is small, the relative error is very high.

\begin{figure}[tb]
	\minipage{0.49\textwidth}
	\textbf{a}
	\includegraphicsmaybe[width=\linewidth]{NN_u_sol_t02.png}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	\textbf{b}
	\includegraphicsmaybe[width=\linewidth]{NN_u_sol_t075.png}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	\textbf{c}
	\includegraphicsmaybe[width=\linewidth]{NN_u_sol_t150.png}
	\endminipage\hfill
	\minipage{0.49\textwidth}
	\textbf{d}
	\includegraphicsmaybe[width=\linewidth]{NN_u_sol_t150_zoomed.png}
	\endminipage\hfill
	\caption{Solution of the differential equation, with explicit solvers with 2 step sizes, the neural network, and the analytical solution. \textbf{a}: Solution at $T=0.2$. \textbf{b}: Solution at $T=0.75$. \textbf{c}: Solution at $T=1.5$. \textbf{d}: Solution at $T=1.5$, zoomed in on the scale of the analytical solution.}
	\label{fig:compare}
\end{figure}

While possible to solve this PDE with a neural network, this is more complex, less accurate and more inefficient than an explicit numerical solution. While the explicit solver with the smallest step size finished in 2 seconds, 40 minutes was needed to train the neural network to get these results, and then the higher step size was used, so using the same step size would have been completely non-viable. For more complex, non-linear PDEs, the neural network solver might be a better tool than explicit schemes, though this was not the case here.
...
Using \eqref{eq:eigval}, the corresponding eigenvalue was $$\lambda = 0.05587217366620945.$$ This is similar to the smallest (in absolute terms) eigenvector, $\lambda_4$. Using the non-rounded value, the log of the relative error was $-4.97$. The log of the largest relative error of each element in the computed eigenvector was $-1.33$.

Each component of the of the computed eigenvector was plotted as a function of epoch, to see how fast it converges. This is shown in Figure \ref{fig:eigvec_converge}. Most of the elements seem to have converged already after about 30 epochs, though then the relative error of the eigenvalue was bigger, with the log of it at $-3.25$. After 100 epochs, it was even better, at $-5.94$, though this seems to be close to a limit, as after 200 epochs, it was only marginarlly better, at $-5.95$.

\begin{figure}
	\centering
	\includegraphicsmaybe[width=\linewidth]{NN_eig_vec_conv.png}
	\caption{}
	\label{fig:eigvec_converge}
\end{figure}

While for many purposes the value obtained here might be accurate enough, it is in general not a great result, considering the process needed to find optimal parameters, and even after that the process is slower than the eigensolver from the standard library. Further, this method is only good for finding a \textit{single} eigenvalue and corresponding vector. While neural networks is a good method for solving the differential equation for finding a eigenvector of a symmetric matrix, this is not a good method for finding a  eigenvector of a symmetric matrix. It would be benefitial if the neural network was generalisable to find an eigenvalue of a symmmetric matrix without needing to be retrained, though if this is case was not tested. Such analyis is outside the scope of this paper, and left for future work.

Theorem 4 in \cite{symmetric} states that the solution should converge to the eigenvector corresponding to the largest eigenvalue of $A$. We got the opposite, having it converge to the smallest. To determine if this was always the case, 1000 different symmetric matrices and start vectors $x(0)$ was generated. Table \ref{tab:idx_cnt} shows how what percentage each rank of eigenvalue was observed, with 1 correspondig to the smallest, 6 to the largest, and None meaning the training resulted in overflow and thus no result. The middle column presents the count for each rank when the matrix $A$ was changed every time, while the right column shows the result keeping the matrix $A$ constant. In both cases, the results are similar, so the choice of $A$ affects the rank less than the choice of $x(0)$. This was to be expected. In both cases, about half the eigenvalues was the smallest, with the frequency decreasing for higher ranks. The properties of the vectors giving the different ranks were not studied.
...
A neural network was used to numerically solve a partial differential equation. While the results obtained were close to the analytic solution, explicit solvers performed much better, and being much more efficient. For more complex PDEs, for instant non-linear ones, the explicit schemes can be difficult to set up, so a neural network might be the best tool.
