...
\subsection{Differential equation for eigenvector of symmetrix matrices}
...
\end{equation*}
for $t>0$ are the eigenvectors of the symmetric $n\times n$ matrix $A$. $x(0)$ is any vector $\in \mathbb{R}^n$.
...
%This is likely the reason why this method was found to be worse than when a single crossover-point was used, since in the latter case, the expression is fairly likely to be passed on unaltered from one parent, the other parent only contributing with genes at the end of the chromosome which are never used for decoding. 
