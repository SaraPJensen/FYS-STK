...
, and thus most will not be recounted here, only the parameters what was then found to be interesting, and the differences made in the new implementation.

While the best method for determining the best parameters for a neural network if searching the entire hyperparameter-space, this is very time-consuming, and will spend a lot of time on some parameter values giving quite bad results. Therefore, a sort of manual grid-search was performed, one parameter at a time. Setting all parameters to some reasonable value such that a result is obtained, they was in turn tweaked one by one until an optimal value was found for the set of other parameters.  This will not ensure the globally optimal parameter set, 
The $L_2$-regularization 
In \cite{p2HO}, the choice of activation function was found to be especially important for getting good results. 
