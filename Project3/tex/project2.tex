...
When solving the differential equation \eqref{eq:diff}, both the explicit scheme (xxx ref expl scheme), and the neural network were used. The results were compared to the analytical solution. For the explicit scheme, both $\Delta x=0.1$ and $\Delta x=0.01$ were used, with $\Delta t$ determined by the stability limit (xxx ref stability limit). The same stabiliy criteria is not appliable to the neural network, so
